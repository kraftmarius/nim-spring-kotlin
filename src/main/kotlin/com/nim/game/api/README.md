...
- **Request Body (Optional):**
  ...
  ```
  - `heaps`: Optional list of heap sizes. A single-element list (e.g. `[13]`) is the classic Nim setup. Multi-heap lists (e.g. `[3, 4, 5]`) are only supported for `I_AM_TOO_YOUNG_TO_DIE` difficulty; using them with `HURT_ME_PLENTY` or `NIGHTMARE` returns `501 Not Implemented`.
